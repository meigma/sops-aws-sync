...
class StageGhdReleaseAssetsTest(unittest.TestCase):
    def test_stages_expected_assets(self) -> None:
        with fixture() as root:
            ...
            self.assertEqual(
                ...
                [
                    "checksums.txt",
                    "sops-aws-sync_1.2.3_darwin_amd64",
                    "sops-aws-sync_1.2.3_darwin_amd64.sbom.json",
                    "sops-aws-sync_1.2.3_darwin_arm64",
                    "sops-aws-sync_1.2.3_darwin_arm64.sbom.json",
                    "sops-aws-sync_1.2.3_linux_amd64",
                    "sops-aws-sync_1.2.3_linux_amd64.sbom.json",
                    "sops-aws-sync_1.2.3_linux_arm64",
                    "sops-aws-sync_1.2.3_linux_arm64.sbom.json",
                ],
            )
            linux_binary = root / "dist/release-assets/sops-aws-sync_1.2.3_linux_amd64"
            mode = linux_binary.stat().st_mode
            self.assertTrue(mode & stat.S_IXUSR)
            self.assertIn("dist/release-assets/sops-aws-sync_1.2.3_linux_arm64", stdout)

    def test_fails_on_missing_checksum_entry(self) -> None:
        with fixture(missing_checksum="sops-aws-sync_1.2.3_linux_arm64") as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("missing checksum entry", stderr)
            self.assertIn("sops-aws-sync_1.2.3_linux_arm64", stderr)

    def test_fails_on_checksum_mismatch(self) -> None:
        override = ("sops-aws-sync_1.2.3_linux_amd64", "0" * 64)
        with fixture(checksum_override=override) as root:
            result, _, stderr = run_script(root)

            self.assertEqual(result, 1)
            self.assertIn("checksum mismatch for sops-aws-sync_1.2.3_linux_amd64", stderr)
    ...
    def test_fails_on_missing_os_arch_asset(self) -> None:
        with fixture(omit_artifact=("linux", "arm64", "Binary")) as root:
            ...
            self.assertEqual(result, 1)
            self.assertIn("missing expected binary asset sops-aws-sync_1.2.3_linux_arm64", stderr)
...
def run_script(root: Path) -> tuple[int, str, str]:
    stdout = io.StringIO()
    stderr = io.StringIO()
    with working_directory(root), github_repository("meigma/sops-aws-sync"):
...
def fixture(
    *,
    signer: str = "meigma/sops-aws-sync/.github/workflows/attest.yml",
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        for goos, goarch in PLATFORMS:
            binary_name = f"sops-aws-sync_1.2.3_{goos}_{goarch}"
        ...
        if extra_binary:
            extra_name = "sops-aws-sync_1.2.3_freebsd_amd64"
...
os = "darwin"
arch = "amd64"
pattern = "sops-aws-sync_${{version}}_darwin_amd64"

[[packages.assets]]
os = "darwin"
arch = "arm64"
pattern = "sops-aws-sync_${{version}}_darwin_arm64"

[[packages.assets]]
os = "linux"
arch = "amd64"
pattern = "sops-aws-sync_${{version}}_linux_amd64"

[[packages.assets]]
os = "linux"
arch = "arm64"
pattern = "sops-aws-sync_${{version}}_linux_arm64"
