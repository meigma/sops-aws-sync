...
- **Content outline (organized to mirror the product; the Action-forwards-to-CLI reality is the reason for a single unified surface, not two parallel docs):**
  - Subcommands: `plan` (read-only; supports plan-only `--detailed-exit-code`), `sync` (always mutates and always verifies — no skip-verify mode; note it lacks an Args validator and silently ignores stray positional arguments), `version` (loads no config or AWS); the bare root prints help.
  ...
  - Runner requirements: node24 runtime; Linux/macOS on X64/ARM64 only; required env `GITHUB_WORKSPACE`, `RUNNER_TEMP`, `RUNNER_OS`, `RUNNER_ARCH` (plus `GITHUB_SHA`/`GITHUB_REPOSITORY` unless `revision`/`repository-id` are supplied).
